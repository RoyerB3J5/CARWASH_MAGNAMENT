...
const updateService = async (req, res) => {
  ...
  try {
    ...
    if (!existingService || JSON.stringify(existingService.service) !== JSON.stringify(service)) {
      ...
      return res.status(200).send({ message: 'El servicio ya existe y no ha sido modificado' });
    }
  } catch (error) {
    return res.status(500).send({ message: error.message });
  }
...
export {getServices, createService, updateService, deleteVehicle,getServicesVehicle,getAllVehicleTypes}